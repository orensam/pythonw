...
class Tableau(object):
    ...
    def _calc_hand_score_simple(self, hand):
        ...
        if self.__ace_base_val in hand_values:
            ...
            # Therefore if there's an ace, we want to try and switch it
            # to 11 and re-calculate.
    ...
    def calc_score_advanced(self):
        ...
        for values_option in self._gen_value_options(self._to_values()):
            # Iterate the possible value arrays,
            # and return the maximum possible.
            score = sum([self._calc_hand_score_advanced(hand, values_option)
                         for hand in self.__hands])
    ...
    def _gen_value_options(self, values):
        ...
        i.e genrates all the combinations of value lists for the
        two ace values. e.g if values is [1,3,1], the output will be:
        [ [1,3,1], [1,3,11], [11,3,1], [11,3,11] ]
        """
        if len(values) == 0:
            yield []
        else:
...
class Spot(object):
    ...
    def __init__(self, pos, card=None):
        """
        Initializes the spot with position pos
        and given card (None for empty spot)
        """
        self.pos = pos
        self.card = card
